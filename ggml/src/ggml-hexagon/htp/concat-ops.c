...
static void concat_generic(unsigned int nth, unsigned int ith, void * data) {
    ...
    const uint32_t type_size = (dst->type == HTP_TYPE_F32) ? 4 : 2;
...
int op_concat(struct htp_ops_context * octx) {
    ...
    bool is_src1_transposed = (src1->nb[1] == (src1->type == HTP_TYPE_F32 ? 4 : 2));
    bool is_src0_untransposed = (src0->nb[0] == (src0->type == HTP_TYPE_F32 ? 4 : 2));
    ...
    if (dim == 0 && is_2d && is_src1_transposed && is_src0_untransposed) {
        uint32_t type_size = (dst->type == HTP_TYPE_F32) ? 4 : 2;
