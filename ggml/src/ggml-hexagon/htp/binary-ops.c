...
static void binary_job_f32_vector_per_thread(struct htp_binary_context * bctx,
    ...
    for (int k = 0; k < 2 && ir_prefetch < end_row; k++) {
        ...
        uint32_t src1_fetch_rows = (ne11 == 1) ? 1 : current_block_size;

        uint8_t * s0_spad = src0_spad_base + spad_idx * src0_spad_half;
        // If ne11=1, use reduced spad size. Half offset is row_size.
        uint8_t * s1_spad = (ne11 == 1)
                            ? (src1_spad_base + spad_idx * src1_row_size_aligned)
                            : (src1_spad_base + spad_idx * src1_spad_half);
        uint8_t * d_spad  = dst_spad_base  + spad_idx * dst_spad_half;
        ...
        // Push Inputs
        dma_queue_push(q, dma_make_ptr(s0_spad, src0_curr), src0_row_size_aligned, nb01, ne00 * sizeof(float), current_block_size);
        dma_queue_push(q, dma_make_ptr(s1_spad, src1_base), src1_row_size_aligned, src1_dma_stride, ne00 * sizeof(float), src1_fetch_rows);
    ...
    for (uint32_t ir = start_row; ir < end_row; ) {
        ...
        uint32_t s1_compute_stride = (ne11 == 1) ? 0 : src1_row_size_aligned;

        // Compute
        for (uint32_t r = 0; r < current_block_size; r++) {
            uint8_t * r_src0 = s0_spad + r * src0_row_size_aligned;
            uint8_t * r_src1 = s1_spad + r * s1_compute_stride;
            uint8_t * r_dst  = d_spad + r * dst_row_size_aligned;
        ...
        if (ir_prefetch < end_row) {
             ...
             uint32_t src1_dma_stride = (ne11 == 1) ? 0 : nb11;
             uint32_t src1_fetch_rows = (ne11 == 1) ? 1 : next_block_size;

             dma_queue_push(q, dma_make_ptr(s0_spad, s0_next), src0_row_size_aligned, nb01, ne00 * sizeof(float), next_block_size);
             dma_queue_push(q, dma_make_ptr(s1_spad, s1_next), src1_row_size_aligned, src1_dma_stride, ne00 * sizeof(float), src1_fetch_rows);
...
static int execute_op_binary_f32(struct htp_ops_context * octx) {
    ...
    bool is_scalar_element = (src1->ne[0] == 1);
    bool is_scalar_row     = (src1->ne[1] == 1);

    // Calculate memory requirements
    // src0, dst always need full block per buffer
    // src1:
    //   if scalar element: 0 (uses scalar path)
    //   if scalar row (and vector element): 1 row per buffer
    //   else: full block per buffer

    // We compute rows_per_buffer (BLOCK_MAX) based on available memory.
    // Total Mem = Threads * 2 * (rows * (src0 + dst) + src1_size)
    // src1_size = is_scalar_element ? 0 : (is_scalar_row ? row_size : rows * row_size)

    // Approximation: Let's assume standard distribution to find BLOCK_MAX, then adjust src1
    size_t row_total_standard = src0_row_size_aligned + dst_row_size_aligned +
                                (is_scalar_element ? 0 : src1_row_size_aligned);

    size_t rows_per_buffer = octx->ctx->vtcm_size / (n_threads * 2 * row_total_standard);
    ...
    if (is_scalar_element) {
        octx->src1_spad.size_per_thread = 0;
    } else if (is_scalar_row) {
        octx->src1_spad.size_per_thread = 2 * src1_row_size_aligned;
    } else {
