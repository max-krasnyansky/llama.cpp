...
/* Legacy 2-letter wrappers mapping to AAA, AAU, UAA, UUU assumptions (though imprecise) */ \
/* Assuming XY means X=dst, Y=src0 (and src1 match src0?) or Y=src1? */ \
/* Based on previous code: aa -> dst(a), src0(a), src1(a) */ \
/* au -> dst(a), src0(a), src1(u) - Wait, previous au had src0 aligned? */ \
/* Let's check: previous au loop used Vector, Vector, UVector. So src0 was aligned. */ \
/* ua -> dst(u), src0(a), src1(a). */ \
/* uu -> dst(u), src0(u), src1(u). */ \
static inline void OP_NAME##_aa(uint8_t * restrict dst, const uint8_t * restrict src0, const uint8_t * restrict src1, uint32_t n) { \
    OP_NAME##_aaa(dst, src0, src1, n); \
} \
static inline void OP_NAME##_au(uint8_t * restrict dst, const uint8_t * restrict src0, const uint8_t * restrict src1, uint32_t n) { \
    OP_NAME##_aau(dst, src0, src1, n); \
} \
static inline void OP_NAME##_ua(uint8_t * restrict dst, const uint8_t * restrict src0, const uint8_t * restrict src1, uint32_t n) { \
    OP_NAME##_uaa(dst, src0, src1, n); \
} \
static inline void OP_NAME##_uu(uint8_t * restrict dst, const uint8_t * restrict src0, const uint8_t * restrict src1, uint32_t n) { \
    OP_NAME##_uuu(dst, src0, src1, n); \
}
...
// Mul-Mul Optimized
// Assuming we keep just aa variant or update if needed.
// Keeping as is for now as it wasn't requested to change deeply.
